...
/*   Updated: 2021/03/07 11:33:12 by ahaddad          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_tmp		*_case_01(t_dolar *dol, char *s, char *tmp1, t_path *path)
{
	char *tmp;

	if ((ft_isalpha2(s[dol->i]) == 1 || ft_isdigit(s[dol->i]) == 1
				|| s[dol->i] == '_') && s[dol->i])
	{
		while ((ft_isalpha2(s[dol->i]) == 1 || ft_isdigit(s[dol->i]) == 1
					|| s[dol->i] == '_') && s[dol->i])
			tmp1[dol->k++] = s[dol->i++];
		tmp1[dol->k] = '\0';
...
t_tmp		*case_01(t_dolar *dol, char *s, char *tmp1, t_path *path)
{
	...
	dol->k = 0;
	dol->c1 = _case_01(dol, s, tmp1, path);
	return (dol->c1);
}
