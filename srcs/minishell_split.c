...
/*   Updated: 2021/03/07 12:35:13 by ahaddad          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char			**colomn1(char **array, char *s, char c, int y)
{
	...
	{
		use.i = 0;
		if (!(array[use.w] = malloc(sizeof(char) *
						(each_char1(s, use.o, c) - use.k + 1))))
			free_2d_char(&(array), use.w);
		use.k = each_char1(s, use.o, c);
		while (s[use.o] && use.o < use.k)
		{
			if (s[use.o] && s[use.o] == c && s[use.o - 1] != '\\' &&
					s[use.o - 1] != '\'' && s[use.o - 1] != '\"')
				use.o++;
