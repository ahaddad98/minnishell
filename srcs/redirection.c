...
/*   Updated: 2021/02/27 15:43:30 by zdnaya           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_list_cmd *pars_red(t_list_cmd *lst, char *red, t_use *use)
{
  ...
  while (red[use->i])
  {
    ...
    if ((red[use->i] == '>' && red[use->i + 1] == '>') || (red[use->i] == '>' || red[use->i] == '<'))
      use->sign = ft_strdup(pars_red_norm(lst, red, use));
  ...
  lst->pipe->all = update_all(use->cmd, use->arg, lst->pipe->all->red);
  if (tmp)
    free(tmp);
  if (use->sign)
    free(use->sign);
...
t_list_cmd *redirection_sort(t_list_cmd *lst, t_use *use, char *free_sp)
{
  ...
  while (free_sp[use->i])
  {
    ...
    if ((free_sp[use->i] == '>' && free_sp[use->i + 1] == '>') || (free_sp[use->i] == '>' || free_sp[use->i] == '<'))
      use->sign = ft_strdup(pars_red_norm(lst, free_sp, use));
  ...
  lst->all = update_all(use->cmd, use->arg, lst->all->red);
  if (tmp)
    free(tmp);
  if (use->sign)
    free(use->sign);
